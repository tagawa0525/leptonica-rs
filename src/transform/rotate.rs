...
/// C `LimitShearAngle` in `rotate.c`: above this a requested shear
/// rotation is downgraded to sampling.
const MAX_SHEAR_ANGLE: f32 = 0.35; // radians, ~20 degrees
...
pub struct RotateOptions {
    ...
    ///
    /// This is C's `width` / `height` pair on `pixRotate`: the dimensions
    /// whose rotation must fit. Passing 0 there means "no embedding", which
    /// is [`RotateEmbed::None`] here.
...
pub enum RotateEmbed {
    /// Do not embed; the output keeps the source dimensions.
    /// C passes `width = height = 0` for this.
    None,
    /// Embed so that a rotation of the **source** dimensions fits.
    /// C passes the source `w, h`.
    #[default]
    ToFit,
    /// Embed so that a rotation of these dimensions fits.
    ///
    /// Repeated rotation needs this: C keeps passing the *original* size on
    /// every iteration, so once the image has grown enough nothing more is
    /// added.
    Explicit(u32, u32),
}
...
pub fn rotate(pix: &Pix, angle: f32, options: &RotateOptions) -> TransformResult<Pix> {
    ...
    // A caller-chosen centre has no counterpart in C's pixRotate (which always
    // rotates about the image centre), so that path keeps the direct
    // implementation below.
    if options.center_x.is_some() || options.center_y.is_some() {
        return rotate_about_arbitrary_center(pix, angle, options);
    }

    let depth = pix.depth();

    // C pixRotate overrides the requested method unconditionally, not just for
    // an "auto" request: 1 bpp always uses shear below Max1BppShearAngle and
    // sampling above it, and any other depth downgrades a large-angle shear to
    // sampling.
    // Only an explicit Auto request is resolved here; C never downgrades a
    // requested area-map rotation for a shallow depth — it converts the image
    // to 8 bpp instead.
    let mut method = if options.method == RotateMethod::Auto {
        select_rotate_method(RotateMethod::Auto, depth, angle)
    } else {
        options.method
    };
    if depth == PixelDepth::Bit1 {
        method = if angle.abs() > MAX_1BPP_SHEAR_ANGLE {
            RotateMethod::Sampling
        } else {
            RotateMethod::Shear
        };
    } else if angle.abs() > MAX_SHEAR_ANGLE && method == RotateMethod::Shear {
        method = RotateMethod::Sampling;
    }
    if method == RotateMethod::Bilinear {
        // No C counterpart; keep the existing implementation for it.
        return rotate_about_arbitrary_center(pix, angle, options);
    }

    // C: remove the colormap when rotating by area mapping.
    let pix1 = if pix.colormap().is_some() && method == RotateMethod::AreaMap {
    ...
    // C: with no embedding (width == 0) the incolor is generated by adding
    // black or white to the colormap.
    let pix1 = if pix1.colormap().is_some() && options.embed == RotateEmbed::None {
        let mut pm = pix1.to_mut();
        if let Some(cmap) = pm.colormap_mut() {
            let _ = cmap.add_black_or_white(options.fill == RotateFill::Black);
        }
        pm.into()
    } else {
        pix1
    ...
    // C: pixEmbedForRotation(pix1, angle, incolor, width, height), where the
    // reg tests pass the source dimensions to request embedding and 0 to skip.
    let (embed_w, embed_h) = match options.embed {
        RotateEmbed::None => (0, 0),
        RotateEmbed::ToFit => (pix1.width(), pix1.height()),
        RotateEmbed::Explicit(w, h) => (w, h),
    };
    let pix2 = embed_for_rotation(&pix1, angle, options.fill, embed_w, embed_h)?;

    // C: area mapping needs 8 or 32 bpp.
    let pix3 = if method == RotateMethod::AreaMap && pix2.depth().bits() < 8 {
        pix2.convert_to_8()
            .map_err(|e| TransformError::InvalidParameters(e.to_string()))?
    } else {
        pix2
    ...
    let (w, h) = (pix3.width(), pix3.height());
    match method {
        RotateMethod::Shear => {
            let fill = match options.fill {
                RotateFill::Black => crate::transform::shear::ShearFill::Black,
                _ => crate::transform::shear::ShearFill::White,
            };
            rotate_shear_center(&pix3, angle, fill)
        }
        RotateMethod::Sampling => {
            rotate_by_sampling(&pix3, (w / 2) as i32, (h / 2) as i32, angle, options.fill)
        }
        _ => {
            if pix3.depth() == PixelDepth::Bit8 {
                rotate_am_gray(&pix3, angle, options.fill)
            } else {
                rotate_am_color(&pix3, angle, options.fill)
            }
        }
    }
}

/// The pre-existing direct rotation, kept for the cases C's `pixRotate` does
/// not cover: a caller-supplied rotation centre and the bilinear method.
fn rotate_about_arbitrary_center(
    pix: &Pix,
    angle: f32,
...
pub fn rotate_am_corner(pix: &Pix, angle: f32, fill: RotateFill) -> TransformResult<Pix> {
    ...
    // C: remove the colormap, then unpack to 8 bpp if the depth is below it,
    // so a 1/2/4 bpp input comes back as an 8 bpp rotation rather than a copy.
    let pix1 = if pix.colormap().is_some() {
        pix.remove_colormap(crate::core::pix::RemoveColormapTarget::BasedOnSrc)
            .map_err(|e| TransformError::InvalidParameters(e.to_string()))?
    } else {
        pix.clone()
    };
    let pix2 = if pix1.depth().bits() < 8 {
        pix1.convert_to_8()
            .map_err(|e| TransformError::InvalidParameters(e.to_string()))?
    } else {
        pix1
    ...
    match pix2.depth() {
        PixelDepth::Bit32 => rotate_am_color_corner(&pix2, angle, fill),
        PixelDepth::Bit8 => rotate_am_gray_corner(&pix2, angle, fill),
...
pub fn rotate_shear(
    ...
    use crate::transform::shear::{h_shear, v_shear};

    // C pixRotateShear: a 2-shear below MaxTwoShearAngle, a 3-shear above it.
    if angle.abs() <= MAX_TWO_SHEAR_ANGLE {
        let pix1 = h_shear(pix, cy, angle, fill)?;
        return v_shear(&pix1, cx, angle, fill);
    }

    // C pixRotate3Shear: vshear(angle/2) -> hshear(atan(sin(angle))) ->
    // vshear(angle/2).
    let hangle = (angle as f64).sin().atan() as f32;
    let pix1 = v_shear(pix, cx, angle / 2.0, fill)?;
    let pix2 = h_shear(&pix1, cy, hangle, fill)?;
    v_shear(&pix2, cx, angle / 2.0, fill)
}
...
///
/// `width` and `height` are the dimensions whose rotation must fit; passing
/// 0 for both means "already big enough", so nothing is embedded. This is
/// C's signature, where callers pass either `0, 0` or the source dimensions.
///
...
pub fn embed_for_rotation(
    ...
    // C: maxside from the *requested* width/height, not the pix's own.
    let maxside =
        ((width as f64 * width as f64 + height as f64 * height as f64).sqrt() + 0.5) as u32;
    if w >= maxside && h >= maxside {
        return Ok(pix.deep_clone());
    }

    // Calculate dimensions needed after rotation. C truncates each candidate
    // to an int before taking the maxima.
    let cosa = (angle as f64).cos();
    let sina = (angle as f64).sin();
    let fw = w as f64;
    let fh = h as f64;
    let w1 = ((fw * cosa - fh * sina).abs() + 0.5) as u32;
    let w2 = ((-fw * cosa - fh * sina).abs() + 0.5) as u32;
    let h1 = ((fw * sina + fh * cosa).abs() + 0.5) as u32;
    let h2 = ((-fw * sina + fh * cosa).abs() + 0.5) as u32;
    let wnew = w.max(w1.max(w2));
    let hnew = h.max(h1.max(h2));
    ...
    // C: pixSetBlackOrWhite, which for a colormapped pix appends the extreme
    // to the colormap and fills with its index.
    out_mut.set_black_or_white(match fill {
        RotateFill::Black => crate::core::pix::InitColor::Black,
        _ => crate::core::pix::InitColor::White,
    });
...
pub fn rotate_by_sampling(
    ...
    // C pixSetBlackOrWhite fills the whole dest before sampling; with a
    // colormap that means the index of black/white, not a raw extreme.
    out_mut.set_black_or_white(match fill {
        RotateFill::Black => crate::core::pix::InitColor::Black,
        _ => crate::core::pix::InitColor::White,
    });
    let fill_value = if pix.colormap().is_some() {
        out_mut.get_pixel_unchecked(0, 0)
    } else {
        fill.to_value(depth)
    };
