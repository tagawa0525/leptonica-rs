...
fn do_exact_tests_on_image(pixs: &leptonica_core::Pix, rp: &mut RegParams, label: &str) {
    ...
    // C版: pixBlockBilateralExact(pixs, 10.0, 10.0)  -- test indices 12-15 per image
...
fn bilateral1_reg_exact() {
    ...
    // --- Test with rock.png ---
    // C版: pixs = pixRead("rock.png"); DoTestsOnImage(pixs, rp, 2000);
    let pixs = load_test_image("rock.png").expect("load rock.png");
    do_exact_tests_on_image(&pixs, &mut rp, "rock.png");

    // --- Test with church.png ---
    // C版: pixs = pixRead("church.png"); DoTestsOnImage(pixs, rp, 1500);
    let pixs = load_test_image("church.png").expect("load church.png");
    do_exact_tests_on_image(&pixs, &mut rp, "church.png");

    // --- Test with color-wheel-hue.jpg ---
    // C版: pixs = pixRead("color-wheel-hue.jpg"); DoTestsOnImage(pixs, rp, 1000);
...
///
/// C版: pixBilateralExact(pixs, spatial_kel, range_kel) -- called via pixBlockBilateralExact
#[test]
fn bilateral1_reg_gray_exact() {
    ...
    let h = pixs.height();
    eprintln!(
        "Testing bilateral_gray_exact on dreyfus8.png ({}x{} d={})",
        w,
        h,
        pixs.depth().bits()
    );
    ...
    // C版: range_kel = NULL case in pixBilateralExact
    ...
    eprintln!(
        "  without range kernel: {}x{}",
        result_no_range.width(),
        result_no_range.height()
    );

    // Verify that bilateral filtering preserves edges (edge-aware smoothing)
    // Sample a few pixels: blurred result should be smoothed but not uniformly
...
/// C版: pixBilateral(pixs, spatial_stdev, range_stdev, ncomps, reduction) with reduction=1,2
/// This is the separable approximate bilateral filter -- NOT available in Rust.
///
/// The C test calls pixBilateral with reduction=1 (8 calls) and reduction=2 (4 calls)
/// for each image. These are all skipped since Rust has no equivalent.
#[test]
#[ignore = "C版: pixBilateral() (separable approximate bilateral) -- Rust未実装のためスキップ"]
fn bilateral1_reg_separable() {
    // C版: pixBilateral(pixs, 5.0, 10.0, ncomps, 1)  -- test 0
    // C版: pixBilateral(pixs, 5.0, 20.0, ncomps, 1)  -- test 1
    // C版: pixBilateral(pixs, 5.0, 40.0, ncomps, 1)  -- test 2
    // C版: pixBilateral(pixs, 5.0, 60.0, ncomps, 1)  -- test 3
    // C版: pixBilateral(pixs, 10.0, 10.0, ncomps, 1) -- test 4
    // C版: pixBilateral(pixs, 10.0, 20.0, ncomps, 1) -- test 5
    // C版: pixBilateral(pixs, 10.0, 40.0, ncomps, 1) -- test 6
    // C版: pixBilateral(pixs, 10.0, 60.0, ncomps, 1) -- test 7
    // C版: pixBilateral(pixs, 10.0, 10.0, ncomps, 2) -- test 8
    // C版: pixBilateral(pixs, 10.0, 20.0, ncomps, 2) -- test 9
    // C版: pixBilateral(pixs, 10.0, 40.0, ncomps, 2) -- test 10
    // C版: pixBilateral(pixs, 10.0, 60.0, ncomps, 2) -- test 11
    //
    // All above use the separable approximate bilateral filter (pixBilateral)
    // which is not implemented in Rust. Only the exact version exists.
}
