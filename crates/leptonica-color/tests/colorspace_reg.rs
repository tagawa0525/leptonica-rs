...
//!
//! C版: reference/leptonica/prog/colorspace_reg.c
//! RGB↔HSV, RGB↔Lab, RGB↔YUV 等の色空間変換をテスト。
...
fn colorspace_reg() {
    ...
    // --- Pixel-level tests ---
    eprintln!("=== Pixel-level colorspace conversions ===");

    // Test RGB → gray
    let gray = rgb_to_gray(255, 0, 0);
    rp.compare_values(1.0, if gray > 0 { 1.0 } else { 0.0 }, 0.0);
    eprintln!("  rgb_to_gray(255,0,0) = {}", gray);
    ...
    eprintln!("  RGB → HSV → RGB roundtrip");
    for &(r, g, b) in &[
        (255, 0, 0),
        (0, 255, 0),
        (0, 0, 255),
    ...
    eprintln!("  RGB → Lab → RGB roundtrip");
    ...
    eprintln!("  RGB → XYZ → RGB roundtrip");
    ...
    eprintln!("  RGB → YUV → RGB roundtrip");
    ...
    // --- Image-level tests ---
    eprintln!("=== Image-level colorspace conversions ===");
