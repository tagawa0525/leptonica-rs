...
//!
//! C版: reference/leptonica/prog/colorfill_reg.c
//! 色ベースのフラッドフィル（color_fill, color_fill_from_seed）をテスト。
//!
//! C版でRust未実装の機能:
//! - l_colorfillCreate(pix, nx, ny) -- タイル分割: Rust未実装
//! - pixColorContentByLocation -- タイルベース分析: Rust未実装
//! - makeColorfillTestData -- テストデータ生成: Rust未実装
//! - cf->pixam, cf->pixadb -- デバッグPixa: Rust未実装
...
/// C版: makeSmallTestPix(c1, c2) -- 17x17画像に三角形パターンで2色配置
...
/// Create a test image with 4 color quadrants
...
/// Create random-ish color blocks (simulating makeColorfillTestData)
...
fn colorfill_reg() {
    ...
    // C版: makeSmallTestPix(0x3070A000, 0xA0703000)
    eprintln!("=== Test 1: Small test image ===");
    ...
    // C版: pixColorContentByLocation(cf, 0, 0, 0, 70, 15, 3, 1, 1)
    ...
    match color_fill_from_seed(&pix1, 8, 8, &opts_small) {
        Ok(Some(r)) => {
            ...
            eprintln!("  seed(8,8): {} pixels", r.pixel_count);
        }
        Ok(None) => {
            eprintln!("  seed(8,8): no region (dark pixel or below min_area)");
            rp.compare_values(1.0, 1.0, 0.0); // c1=0x3070A000 may be dark
        }
        Err(e) => {
            eprintln!("  FAILED: {}", e);
    ...
    for &(sx, sy, name) in &[
        ...
        match color_fill_from_seed(&pix_regions, sx, sy, &opts) {
            Ok(Some(r)) => {
                ...
                eprintln!(
                    "  {}: {} pixels, avg=({},{},{})",
                    name, r.pixel_count, r.avg_color.0, r.avg_color.1, r.avg_color.2
                );
            }
            Ok(None) => {
    ...
    // C版: l_colorfillCreate(pix2, 1, 1) + pixColorContentByLocation
    eprintln!("=== Test 3: Full image fill ===");
    match color_fill(&pix_regions, &opts) {
        Ok(r) => {
            ...
            rp.compare_values(100.0, r.mask.height() as f64, 0.0);
            eprintln!(
                "  {} regions, {} total pixels",
                r.region_count, r.total_pixels
            );
        }
        Err(e) => {
            eprintln!("  FAILED: {}", e);
    ...
    // C版: makeColorfillTestData(350, 350, 35, 100)
    eprintln!("=== Test 4: Random-color fill ===");
    ...
    match color_fill(&pix_random, &opts_rand) {
        Ok(r) => {
            ...
            eprintln!("  {} regions, {} pixels", r.region_count, r.total_pixels);
        }
        Err(e) => {
            eprintln!("  FAILED: {}", e);
            rp.compare_values(1.0, 0.0, 0.0);
    ...
    match (r4, r8) {
        (Some(a), Some(b)) => {
            ...
            eprintln!("  4-way: {}, 8-way: {}", a.pixel_count, b.pixel_count);
        }
        _ => {
            for _ in 0..2 {
                rp.compare_values(1.0, 0.0, 0.0);
    ...
    // C版: l_colorfillCreate(pix2, 2, 2) -- 4タイル -- Rust未実装のためスキップ
    // C版: l_colorfillCreate(pix2, 3, 3) -- 9タイル -- Rust未実装のためスキップ
    // C版: lyra.005.jpg テスト -- テスト画像なし、スキップ
