...
fn binmorph2_reg() {
    ...
    // We assert pixel-level equality **in the interior** (after cropping
    // a margin of `2 * max(hsize, vsize)` from every side), which keeps the
    // test's original intent (separability check) while tolerating the
    // documented boundary asymmetry.
    ...
    let margin: u32 = 22; // 2 * max(hsize, vsize) for (11, 11)
    let inner_w = w.saturating_sub(2 * margin);
    let inner_h = h.saturating_sub(2 * margin);
    let de_inner = dilated_eroded
        .clip_rectangle(margin, margin, inner_w, inner_h)
        .expect("clip de");
    let cd_inner = closed_direct
        .clip_rectangle(margin, margin, inner_w, inner_h)
        .expect("clip cd");
    let same = de_inner.equals(&cd_inner);
    rp.compare_values(1.0, if same { 1.0 } else { 0.0 }, 0.0);

    // --- Test: Erosion then dilation vs opening (interior equality) ---
    ...
    let ed_inner = eroded_dilated
        .clip_rectangle(margin, margin, inner_w, inner_h)
        .expect("clip ed");
    let od_inner = opened_direct
        .clip_rectangle(margin, margin, inner_w, inner_h)
        .expect("clip od");
    let same = ed_inner.equals(&od_inner);
    rp.compare_values(1.0, if same { 1.0 } else { 0.0 }, 0.0);
