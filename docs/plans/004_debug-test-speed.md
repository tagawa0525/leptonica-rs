...
`cargo test --all-features` が debug版(259秒) vs release版(14秒) で約18.5倍遅い。
...
| `src/morph/src/dwa.rs` | 水平dilation/erosionをワードシフト版に置換 |
...
# release版の退行確認
time cargo test --all-features --release
