...
C版期待ハッシュの作り方: `scripts/verify_fillmapholes.c` の Prerequisites 通り、ローカルで `reference/leptonica` を clone + cmake build し、verify バイナリを実行して `/tmp/c_fillmapholes_weasel.png` を生成 → `tests/common::params::pixel_content_hash` 相当の関数で FNV-1a ハッシュを採取し、それを定数としてコミットする。
**生成物（PNG・libleptonica.a 等）はリポジトリに追加しない**ため、`reference/leptonica` の clone・build はハッシュ更新時のみ必要。CI や通常の `cargo test` 実行では一切不要で、`tests/golden_manifest.tsv` の運用方針（hash のみ git管理）とも整合する。
...
```bash
$ grep -REn 'fill_map_holes_inner|fill_map_holes\(' src/ | grep -v test
