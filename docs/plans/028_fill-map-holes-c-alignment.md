...
`reference/leptonica/src/adaptmap.c` lines 1495-1581 を Rust に移植する。擬似コードで:
...
- 8bpp Pix のみ受け付ける制約は既存の `fill_map_holes` 入口で保たれている（要確認）
...
- C版出力 PNG を `tests/data/golden/` 配下にチェックインする（`reference/leptonica` は gitignore なので、毎回C版を build する代わりに既知のC出力を bit比較する）
- もしくは: C版出力の FNV-1a ピクセルハッシュを `golden_manifest.tsv` に登録し、Rust出力ハッシュと比較

C版出力は `scripts/verify_fillmapholes.c` を1度実行して `/tmp/c_fillmapholes_weasel.png` を取得 → リポジトリに `tests/data/golden/c_fillmapholes_weasel.png` として保存する方針がシンプル。サイズ ~1KB なので git管理も問題ない。
