...
4. `golden_manifest.tsv` のmorphエントリ数が `write_pix_and_check` + `write_data_and_check` 呼び出し総数と一致
5. C版golden比較: `cargo run --example compare_golden --features all-formats -- --module morph`
