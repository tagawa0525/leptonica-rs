...
- `Pix::uses_cmap_color`: cmap entry index を直接スキャンする実装。
  - C 版の `pixGetGrayHistogram` 経由のロジックは使わない。
  - 理由: Rust の `gray_histogram_colormapped` は cmap entry をgray 値に変換して集計するため、index ベースの判定には使えない。
