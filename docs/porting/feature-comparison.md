...
調査日: 2026-05-10（verify-comparison-counts実行結果を反映）

## 概要

| 項目                 | C版 (reference/leptonica) | Rust版 (leptonica-rs) |
| -------------------- | ------------------------- | --------------------- |
| ソースファイル数     | **182個** (.c)            | **180個** (.rs)       |
| コード行数           | **約249,000行**           | **約147,000行**       |
| 実装率（行数ベース） | 100%                      | **約59%**             |
...
| ----------------------------------------------------- | --------- | --------- | --------- | ------- | --------- | ---------- | ------------ |
| [leptonica (src/core/)](comparison/core.md)           | 864       | 94        | 111       | 146     | 1,215     | 78.8%      | 89.6%        |
| [leptonica (src/io/)](comparison/io.md)               | 139       | 19        | 0         | 50      | 208       | 76.0%      | 100.0%       |
| [leptonica (src/transform/)](comparison/transform.md) | 109       | 19        | 0         | 14      | 142       | 90.1%      | 100.0%       |
| [leptonica (src/morph/)](comparison/morph.md)         | 116       | 22        | 0         | 33      | 171       | 80.7%      | 100.0%       |
| [leptonica (src/filter/)](comparison/filter.md)       | 117       | 0         | 1         | 13      | 131       | 89.3%      | 99.2%        |
| [leptonica (src/color/)](comparison/color.md)         | 102       | 20        | 0         | 17      | 139       | 87.8%      | 100.0%       |
| [leptonica (src/region/)](comparison/region.md)       | 65        | 8         | 0         | 22      | 95        | 76.8%      | 100.0%       |
| [leptonica (src/recog/)](comparison/recog.md)         | 134       | 45        | 10        | 18      | 207       | 86.5%      | 94.7%        |
| [その他](comparison/misc.md)                          | 145       | 5         | 5         | 371     | 526       | 28.5%      | 96.8%        |
| **合計**                                              | **1,791** | **232**   | **127**   | **684** | **2,834** | **71.4%**  | **94.1%**    |
...
| 機能                   | C版                           | Rust版                   | 備考                                                |
| ---------------------- | ----------------------------- | ------------------------ | --------------------------------------------------- |
| Pix（画像コンテナ）    | ✅ pix1-5.c                   | ✅ leptonica (src/core/) | 基本操作・深度変換（1/2/4/8/16/32bpp 相互変換）実装 |
...
| TIFF                  | ✅ tiffio.c               | ✅ tiff.rs   | feature gate (`tiff-format`)、マルチページ対応、**1bpp 直接書き出し対応 (PR #383)** + **C 慣習に揃った PhotometricInterpretation 解釈 (PR #389)** |
...
| 機能              | C版                          | Rust版           | 備考                                                                                               |
| ----------------- | ---------------------------- | ---------------- | -------------------------------------------------------------------------------------------------- |
| 回転（直交）      | ✅ rotateorth.c              | ✅ rotate.rs     | 90/180/270 度                                                                                      |
| 回転（任意角度）  | ✅ rotate.c, rotateam.c      | ✅ rotate.rs     | 面積マッピング/サンプリング/シアー                                                                 |
| 回転（シアー）    | ✅ rotateshear.c             | ✅ rotate.rs     | 2-shear/3-shear対応                                                                                |
| スケーリング      | ✅ scale1-2.c                | ✅ scale.rs      | 3アルゴリズム＋1bpp特化（`scale_binary`/`scale_to_gray`/`expand_binary_*`/`reduce_rank_binary_*`） |
...
| 構造化要素（SEL）    | ✅ sel1-2.c, selgen.c      | ✅ sel.rs, selgen.rs | 基本＋自動生成（`selgen`）＋Sela 永続化I/O（`Sela::read/write`） |
...
| バイラテラルフィルタ | ✅ bilateral.c | ✅ bilateral.rs | エッジ保存平滑化＋PBC分離型による高速近似 |
...
| デワーピング             | ✅ dewarp1-4.c                | ✅ dewarp/     | 単一ページ＋Dewarpa（多ページ管理） |
...
| -------------------------- | ------------ | ----------------------- | ----------------------- | ------------------------------------------------------------------------------------ |
| leptonica (src/core/)      | ~47,100      | 958/1,215 (78.8%)       | 958/1,069 (89.6%)       | Pix, Box, Pta, Ptaa, Pixaa, Colormap, 演算, 比較, ブレンド, 描画, 統計, ヒストグラム |
| leptonica (src/io/)        | ~7,900       | 158/208 (76.0%)         | 158/158 (100.0%)        | BMP/PNG/JPEG/PNM/TIFF/GIF/WebP/JP2K/PDF/PS/SPIX + ヘッダー読み取り                   |
| leptonica (src/transform/) | ~11,200      | 128/142 (90.1%)         | 128/128 (100.0%)        | 回転, スケーリング, アフィン, 射影, シアー                                           |
| leptonica (src/morph/)     | ~9,400       | 138/171 (80.7%)         | 138/138 (100.0%)        | 二値/グレースケール/カラー形態学, DWA, 細線化                                        |
| leptonica (src/filter/)    | ~9,800       | 117/131 (89.3%)         | 117/118 (99.2%)         | 畳み込み, エッジ検出, バイラテラル, ランク, 適応マッピング, ランレングス             |
| leptonica (src/color/)     | ~7,400       | 122/139 (87.8%)         | 122/122 (100.0%)        | 色空間変換, 量子化, セグメンテーション, 二値化, 色分析, カラーマップ塗装             |
| leptonica (src/region/)    | ~10,600      | 73/95 (76.8%)           | 73/73 (100.0%)          | 連結成分, シードフィル, 分水嶺, 四分木, 迷路                                         |
| leptonica (src/recog/)     | ~16,000      | 179/207 (86.5%)         | 179/189 (94.7%)         | スキュー補正, デワーピング, 文字認識, バーコード                                     |
| その他                     | -            | 150/526 (28.5%)         | 150/155 (96.8%)         | ワーパー, エンコーディング, デバッグ/タイミング系等                                  |
| **合計**                   | **~147,000** | **2,023/2,834 (71.4%)** | **2,023/2,150 (94.1%)** |                                                                                      |

## 未実装関数の状況

🚫 不要と分類された 684 関数を除く 2,150 関数のうち 2,023 関数が実装済み（✅ 1,791 + 🔄 232）。
残り 127 関数（❌）は未実装。実カバレッジは 94.1%。

本数値は `gap-fill audit 2026-05-10` で `allheaders.h` の C 公開関数 2,743 件を全数集計し、
さらに各 ✅/🔄/🚫/❌ エントリを C 関数名・C ソースファイル・Rust 実装場所で個別レビューした
結果を反映している。Numa/Pixa 等の core データ構造は idiomatic Rust API (`push`/`len`/`clone` 等)
として再分類されており、🔄 件数が大幅に増加した。Rust 標準ライブラリで代替可能な
ファイル/パス/文字列ユーティリティ等は 🚫 に再分類した。
追加検証エントリは各 `comparison/*.md` 末尾の「追加検証エントリ」セクションを参照。
...
- [leptonica (src/transform/)](comparison/transform.md) — 142関数（🚫14不要）
- [leptonica (src/morph/)](comparison/morph.md) — 163関数（🚫25不要）
- [leptonica (src/filter/)](comparison/filter.md) — 120関数（🚫13不要）
- [leptonica (src/color/)](comparison/color.md) — 139関数（🚫17不要）
- [leptonica (src/region/)](comparison/region.md) — 95関数（🚫22不要）
- [leptonica (src/recog/)](comparison/recog.md) — 182関数（🚫18不要）
- [その他](comparison/misc.md) — 324関数（🚫181不要）
