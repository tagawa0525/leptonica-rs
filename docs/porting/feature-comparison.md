...
|---------|--------|----------|---------|---------|------|-----------|------------|
| [leptonica (src/core/)](comparison/core.md) | 779 | 30 | 33 | 77 | 919 | 88.0% | 96.1% |
| [leptonica (src/io/)](comparison/io.md) | 137 | 18 | 2 | 45 | 202 | 76.7% | 98.7% |
...
| [leptonica (src/filter/)](comparison/filter.md) | 96 | 0 | 11 | 11 | 118 | 81.4% | 89.7% |
...
| [leptonica (src/recog/)](comparison/recog.md) | 122 | 26 | 3 | 18 | 169 | 87.6% | 98.0% |
| [その他](comparison/misc.md) | 100 | 0 | 46 | 177 | 323 | 31.0% | 68.5% |
| **合計** | **1,615** | **133** | **95** | **403** | **2,246** | **77.8%** | **94.8%** |
...
| -------- | ---- | ------------- | ----------- | -------- |
| leptonica (src/core/) | ~47,100 | 809/919 (88.0%) | 809/842 (96.1%) | Pix, Box, Pta, Ptaa, Pixaa, Colormap, 演算, 比較, ブレンド, 描画, 統計, ヒストグラム |
| leptonica (src/io/) | ~7,900 | 155/202 (76.7%) | 155/157 (98.7%) | BMP/PNG/JPEG/PNM/TIFF/GIF/WebP/JP2K/PDF/PS/SPIX + ヘッダー読み取り |
...
| leptonica (src/filter/) | ~9,800 | 96/118 (81.4%) | 96/107 (89.7%) | 畳み込み, エッジ検出, バイラテラル, ランク, 適応マッピング |
...
| leptonica (src/recog/) | ~16,000 | 148/169 (87.6%) | 148/151 (98.0%) | スキュー補正, デワーピング, 文字認識, バーコード |
| その他 | - | 100/323 (31.0%) | 100/146 (68.5%) | ワーパー, エンコーディング |
| **合計** | **~119,500** | **1,748/2,246 (77.8%)** | **1,748/1,843 (94.8%)** | |

## 未実装関数の状況

🚫不要と分類された403関数を除く1,843関数のうち、1,748関数（✅1,615 + 🔄133）が実装済み。
❌未実装は95関数で、実カバレッジは94.8%。主な未実装領域:
- **core/boxfunc2,5**: Box配列の変換・ソート・統計関数（33件）
- **filter/kernel**: Kernel I/O・変換関数（11件）
- **misc**: GPlot・BMF・ストローク・ランレングス等（46件）
- **recog/classapp**: 文書分類補助関数（3件）
- **io**: PNG/JPEG一部関数（2件）
...
- [leptonica (src/core/)](comparison/core.md) — 919関数（🚫77不要、❌33未実装）
- [leptonica (src/io/)](comparison/io.md) — 202関数（🚫45不要、❌2未実装）
- [leptonica (src/transform/)](comparison/transform.md) — 137関数（🚫14不要）
- [leptonica (src/morph/)](comparison/morph.md) — 150関数（🚫26不要）
- [leptonica (src/filter/)](comparison/filter.md) — 118関数（🚫11不要、❌11未実装）
- [leptonica (src/color/)](comparison/color.md) — 133関数（🚫13不要）
- [leptonica (src/region/)](comparison/region.md) — 95関数（🚫22不要）
- [leptonica (src/recog/)](comparison/recog.md) — 169関数（🚫18不要、❌3未実装）
- [その他](comparison/misc.md) — 323関数（🚫177不要、❌46未実装）
