...
調査日: 2026-05-20（全C版回帰テスト159個の移植完了。Rust独自テスト 85 件)。

> 関連: [C 互換性ベースライン](c-compat-status.md) — Phase 2 のレポート
> 機構で測定した pixel-level 一致率 (Ok / Mismatch / Unmapped) はこちら。
...
| 回帰テスト     | **160個** (*_reg.c)       | **244個** (*_reg.rs、うち C 対応 159 + 独自 85) |
| 個別テスト関数 | 多数                      | **約4,051個**                                   |
