...
| C関数                    | 状態    | Rust対応                  | 備考                               |
| ------------------------ | ------- | ------------------------- | ---------------------------------- |
| boxaGetWhiteblocks       | ✅ 同等 | `get_whiteblocks`         | ホワイトブロック検出               |
| boxaPruneSortedOnOverlap | ✅ 同等 | `prune_sorted_on_overlap` | オーバーラップに基づくプルーニング |
...
| pixaAddTextNumber        | ✅ 同等 | `pixa_add_text_number`      | テキスト番号付き追加   |
| pixaAddTextlines         | ✅ 同等 | `pixa_add_textlines`        | テキストライン群追加   |
| pixaAddPixWithText       | ✅ 同等 | `pixa_add_pix_with_text`    | テキスト付きPix追加    |
...
| C関数          | 状態    | Rust対応            | 備考                     |
| -------------- | ------- | ------------------- | ------------------------ |
| bmfCreate      | ✅ 同等 | `Bmf::new`          | ビットマップフォント作成 |
| bmfDestroy     | 🚫 不要 | -                   | RustのDrop traitで代替   |
| bmfGetPix      | ✅ 同等 | `Bmf::get_pix`      | フォント画像取得         |
| bmfGetWidth    | ✅ 同等 | `Bmf::get_width`    | フォント幅取得           |
| bmfGetBaseline | ✅ 同等 | `Bmf::get_baseline` | ベースライン取得         |
| pixaGetFont    | ✅ 同等 | `pixa_get_font`     | フォント取得             |
...
| C関数               | 状態    | Rust対応                  | 備考                   |
| ------------------- | ------- | ------------------------- | ---------------------- |
| gplotCreate         | ✅ 同等 | `GPlot::new`              | グラフ作成             |
| gplotDestroy        | 🚫 不要 | -                         | RustのDrop traitで代替 |
| gplotAddPlot        | ✅ 同等 | `GPlot::add_plot`         | プロット追加           |
| gplotSetScaling     | ✅ 同等 | `GPlot::set_scaling`      | スケーリング設定       |
| gplotMakeOutputPix  | ✅ 同等 | `GPlot::make_output_pix`  | 出力Pix生成            |
| gplotMakeOutput     | ✅ 同等 | `GPlot::make_output`      | 出力ファイル生成       |
| gplotGenCommandFile | ✅ 同等 | `GPlot::gen_command_file` | コマンドファイル生成   |
| gplotGenDataFiles   | ✅ 同等 | `GPlot::gen_data_files`   | データファイル生成     |
...
| pixaFindStrokeWidth   | ✅ 同等 | `pixa_find_stroke_width`   | 複数線幅検出 |
| pixaModifyStrokeWidth | ✅ 同等 | `pixa_modify_stroke_width` | 線幅変更     |
| pixModifyStrokeWidth  | ✅ 同等 | `modify_stroke_width`      | 単一線幅変更 |
| pixaSetStrokeWidth    | ✅ 同等 | `pixa_set_stroke_width`    | 線幅設定     |
...
| makeMSBitLocTab           | ✅ 同等 | `make_msbit_loc_tab`           | MSBビット位置テーブル生成            |
...
| C関数              | 状態    | Rust対応                | 備考                 |
| ------------------ | ------- | ----------------------- | -------------------- |
| convertFilesTo1bpp | ✅ 同等 | `convert_files_to_1bpp` | ファイルを1bppに変換 |
