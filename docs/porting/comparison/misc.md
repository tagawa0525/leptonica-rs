...
**カバレッジ**: 28.5% (150/526 関数が実装済み、🚫 不要 305 関数を除くと実質 150/221 = 67.9% 実装)
...
**追加分類サマリー**: ✅ 7 / 🚫 124 / ❌ 71 (合計 202)

### allheaders.h (追加分)

| C関数                | 状態 | Rust対応 | 備考                          |
| -------------------- | ---- | -------- | ----------------------------- |
| leptSetStderrHandler | ❌   | -        | no Rust impl in expected dirs |
...
| C関数        | 状態 | Rust対応 | 備考                          |
| ------------ | ---- | -------- | ----------------------------- |
| pixaSaveFont | ❌   | -        | no Rust impl in expected dirs |
...
| numaConvertToDna        | ❌   | -        | no Rust impl in expected dirs    |
| pixConvertDataToDna     | ❌   | -        | no Rust impl in expected dirs    |
...
| C関数          | 状態 | Rust対応 | 備考                          |
| -------------- | ---- | -------- | ----------------------------- |
| parseForProtos | ❌   | -        | no Rust impl in expected dirs |

### pdfapp.c (追加分)

| C関数                | 状態 | Rust対応 | 備考                          |
| -------------------- | ---- | -------- | ----------------------------- |
| rotateorthFilesToPdf | ❌   | -        | no Rust impl in expected dirs |
...
| l_hmapCreateFromPta   | ❌   | -                                  | no Rust impl in expected dirs |
| ptaIntersectionByAset | ❌   | -                                  | no Rust impl in expected dirs |
| ptaIntersectionByHmap | ❌   | -                                  | no Rust impl in expected dirs |
| ptaRemoveDupsByAset   | ❌   | -                                  | no Rust impl in expected dirs |
| ptaRemoveDupsByHmap   | ❌   | -                                  | no Rust impl in expected dirs |
| ptaUnionByAset        | ❌   | -                                  | no Rust impl in expected dirs |
| ptaUnionByHmap        | ❌   | -                                  | no Rust impl in expected dirs |
...
| C関数                   | 状態 | Rust対応 | 備考                          |
| ----------------------- | ---- | -------- | ----------------------------- |
| regTestGenLocalFilename | ❌   | -        | no Rust impl in expected dirs |

### roplow.c (追加分)

| C関数          | 状態 | Rust対応 | 備考                          |
| -------------- | ---- | -------- | ----------------------------- |
| rasteropHipLow | ❌   | -        | no Rust impl in expected dirs |
| rasteropLow    | ❌   | -        | no Rust impl in expected dirs |
| rasteropUniLow | ❌   | -        | no Rust impl in expected dirs |
| rasteropVipLow | ❌   | -        | no Rust impl in expected dirs |
...
| l_hmapCreateFromSarray   | ❌   | -                                        | no Rust impl in expected dirs |
| sarrayGenerateIntegers   | ✅   | `generate_integers` (core/sarray/mod.rs) | name+module match             |
| sarrayIntersectionByAset | ❌   | -                                        | no Rust impl in expected dirs |
| sarrayIntersectionByHmap | ❌   | -                                        | no Rust impl in expected dirs |
| sarrayLookupCSKV         | ❌   | -                                        | no Rust impl in expected dirs |
| sarrayRemoveDupsByAset   | ❌   | -                                        | no Rust impl in expected dirs |
| sarrayRemoveDupsByHmap   | ❌   | -                                        | no Rust impl in expected dirs |
| sarrayUnionByAset        | ❌   | -                                        | no Rust impl in expected dirs |
| sarrayUnionByHmap        | ❌   | -                                        | no Rust impl in expected dirs |
| stringCompareLexical     | ❌   | -                                        | no Rust impl in expected dirs |

### stringcode.c (追加分)

| C関数                  | 状態 | Rust対応 | 備考                          |
| ---------------------- | ---- | -------- | ----------------------------- |
| l_getStructStrFromFile | ❌   | -        | no Rust impl in expected dirs |

### textops.c (追加分)

| C関数                   | 状態 | Rust対応 | 備考                          |
| ----------------------- | ---- | -------- | ----------------------------- |
| pixAddSingleTextblock   | ❌   | -        | no Rust impl in expected dirs |
| splitStringToParagraphs | ❌   | -        | no Rust impl in expected dirs |
...
| genRandomIntOnInterval   | ❌   | -        | no Rust impl in expected dirs         |
| l_getCurrentTime         | ❌   | -        | no Rust impl in expected dirs         |
| l_getFormattedDate       | ❌   | -        | no Rust impl in expected dirs         |
...
| lept_ceiling             | ❌   | -        | no Rust impl in expected dirs         |
| lept_floor               | ❌   | -        | no Rust impl in expected dirs         |
| lept_isPrime             | ❌   | -        | no Rust impl in expected dirs         |
| lept_roundftoi           | ❌   | -        | no Rust impl in expected dirs         |
...
| setMsgSeverity           | ❌   | -        | no Rust impl in expected dirs         |
| startTimer               | 🚫   | -        | デバッグ/タイミング系                 |
| startTimerNested         | 🚫   | -        | デバッグ/タイミング系                 |
| startWallTimer           | ❌   | -        | no Rust impl in expected dirs         |
| stopTimer                | 🚫   | -        | デバッグ/タイミング系                 |
| stopTimerNested          | 🚫   | -        | デバッグ/タイミング系                 |
| stopWallTimer            | ❌   | -        | no Rust impl in expected dirs         |
...
| arrayFindEachSequence     | ❌   | -                                          | no Rust impl in expected dirs      |
| arrayFindSequence         | ✅   | `array_find_sequence` (core/sarray/mod.rs) | name+module match                  |
| arrayReplaceEachSequence  | ❌   | -                                          | no Rust impl in expected dirs      |
...
| l_binaryCompare           | ❌   | -                                          | no Rust impl in expected dirs      |
| l_binaryCopy              | ❌   | -                                          | no Rust impl in expected dirs      |
| l_binaryRead              | ❌   | -                                          | no Rust impl in expected dirs      |
| l_binaryReadSelect        | ❌   | -                                          | no Rust impl in expected dirs      |
| l_binaryReadSelectStream  | ❌   | -                                          | no Rust impl in expected dirs      |
| l_binaryReadStream        | ❌   | -                                          | no Rust impl in expected dirs      |
| l_binaryWrite             | ❌   | -                                          | no Rust impl in expected dirs      |
| l_makeTempFilename        | ❌   | -                                          | no Rust impl in expected dirs      |
| lept_calloc               | 🚫   | -                                          | Rust の Box/Vec で代替             |
| lept_cp                   | ❌   | -                                          | no Rust impl in expected dirs      |
| lept_direxists            | ❌   | -                                          | no Rust impl in expected dirs      |
| lept_fclose               | ❌   | -                                          | no Rust impl in expected dirs      |
| lept_fopen                | ❌   | -                                          | no Rust impl in expected dirs      |
| lept_free                 | 🚫   | -                                          | Rust の Box/Vec で代替             |
| lept_mkdir                | ❌   | -                                          | no Rust impl in expected dirs      |
| lept_mv                   | ❌   | -                                          | no Rust impl in expected dirs      |
| lept_rm                   | ❌   | -                                          | no Rust impl in expected dirs      |
| lept_rm_match             | ❌   | -                                          | no Rust impl in expected dirs      |
| lept_rmdir                | ❌   | -                                          | no Rust impl in expected dirs      |
| lept_rmfile               | ❌   | -                                          | no Rust impl in expected dirs      |
| makeTempDirname           | ❌   | -                                          | no Rust impl in expected dirs      |
| modifyTrailingSlash       | ❌   | -                                          | no Rust impl in expected dirs      |
| nbytesInFile              | ❌   | -                                          | no Rust impl in expected dirs      |
| pathJoin                  | 🚫   | -                                          | Rust 標準の std::path で代替       |
| reallocNew                | ❌   | -                                          | no Rust impl in expected dirs      |
| splitPathAtDirectory      | 🚫   | -                                          | Rust 標準の std::path で代替       |
| splitPathAtExtension      | 🚫   | -                                          | Rust 標準の std::path で代替       |
| stringCat                 | ❌   | -                                          | no Rust impl in expected dirs      |
| stringCheckForChars       | ❌   | -                                          | no Rust impl in expected dirs      |
| stringConcatNew           | ❌   | -                                          | no Rust impl in expected dirs      |
| stringCopy                | ❌   | -                                          | no Rust impl in expected dirs      |
| stringCopySegment         | ❌   | -                                          | no Rust impl in expected dirs      |
...
| stringLength              | ❌   | -                                          | no Rust impl in expected dirs      |
...
| strtokSafe                | ❌   | -                                          | no Rust impl in expected dirs      |
