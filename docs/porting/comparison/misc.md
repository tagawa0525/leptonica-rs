...
| C関数 | 状態 | Rust対応 | 備考 |
|-------|------|----------|------|
| boxaGetWhiteblocks | ✅ 同等 | `Boxa::get_whiteblocks` | ホワイトブロック検出 |
| boxaPruneSortedOnOverlap | ✅ 同等 | `Boxa::prune_sorted_on_overlap` | オーバーラップに基づくプルーニング |
...
| C関数 | 状態 | Rust対応 | 備考 |
|-------|------|----------|------|
| partifyFiles | ✅ 同等 | `partify_files` | ファイル分割 |
| partifyPixac | ✅ 同等 | `partify_pixac` | Pixac分割 |
...
| pixReduceBinary2 | ✅ 同等 | `reduce_binary2` | 2倍縮小 |
...
**binexpand.c / binreduce.c カバレッジ**: 6/6 = 100%
...
| C関数 | 状態 | Rust対応 | 備考 |
|-------|------|----------|------|
| pixAddTextlines | ✅ 同等 | `add_textlines` | テキストライン追加 |
| pixSetTextblock | ✅ 同等 | `set_textblock` | テキストブロック設定 |
| pixSetTextline | ✅ 同等 | `set_textline` | テキストライン設定 |
| pixaAddTextNumber | ✅ 同等 | `Pixa::add_text_number` | テキスト番号付き追加 |
| pixaAddTextlines | ✅ 同等 | `Pixa::add_textlines` | テキストライン群追加 |
| pixaAddPixWithText | ✅ 同等 | `Pixa::add_pix_with_text` | テキスト付きPix追加 |
...
| bmfGetLineStrings | ✅ 同等 | `bmf_get_line_strings` | ライン文字列取得 |
| bmfGetWordWidths | ✅ 同等 | `bmf_get_word_widths` | 単語幅取得 |
| bmfGetStringWidth | ✅ 同等 | `bmf_get_string_width` | 文字列幅取得 |

**textops.c カバレッジ**: 19/19 = 100%

### bmf.c (ビットマップフォント)
| C関数 | 状態 | Rust対応 | 備考 |
|-------|------|----------|------|
| bmfCreate | ✅ 同等 | `bmf_create` | ビットマップフォント作成 |
| bmfDestroy | 🚫 不要 | - | RustのDrop traitで代替 |
| bmfGetPix | ✅ 同等 | `bmf_get_pix` | フォント画像取得 |
| bmfGetWidth | ✅ 同等 | `bmf_get_width` | フォント幅取得 |
| bmfGetBaseline | ✅ 同等 | `bmf_get_baseline` | ベースライン取得 |
| pixaGetFont | ✅ 同等 | `Pixa::get_font` | フォント取得 |
...
| C関数 | 状態 | Rust対応 | 備考 |
|-------|------|----------|------|
| gplotCreate | ✅ 同等 | `gplot_create` | グラフ作成 |
| gplotDestroy | 🚫 不要 | - | RustのDrop traitで代替 |
| gplotAddPlot | ✅ 同等 | `gplot_add_plot` | プロット追加 |
| gplotSetScaling | ✅ 同等 | `gplot_set_scaling` | スケーリング設定 |
| gplotMakeOutputPix | ✅ 同等 | `gplot_make_output_pix` | 出力Pix生成 |
| gplotMakeOutput | ✅ 同等 | `gplot_make_output` | 出力ファイル生成 |
| gplotGenCommandFile | ✅ 同等 | `gplot_gen_command_file` | コマンドファイル生成 |
| gplotGenDataFiles | ✅ 同等 | `gplot_gen_data_files` | データファイル生成 |
| gplotSimple1 | ✅ 同等 | `gplot_simple1` | 単純グラフ1 |
| gplotSimple2 | ✅ 同等 | `gplot_simple2` | 単純グラフ2 |
| gplotSimpleN | ✅ 同等 | `gplot_simple_n` | 複数グラフ |
| gplotSimplePix1 | ✅ 同等 | `gplot_simple_pix1` | Pix単純グラフ1 |
| gplotSimplePix2 | ✅ 同等 | `gplot_simple_pix2` | Pix単純グラフ2 |
| gplotSimplePixN | ✅ 同等 | `gplot_simple_pix_n` | Pix複数グラフ |
...
| C関数 | 状態 | Rust対応 | 備考 |
|-------|------|----------|------|
| pixFindStrokeLength | ✅ 同等 | `find_stroke_length` | 線長検出 |
| pixFindStrokeWidth | ✅ 同等 | `find_stroke_width` | 線幅検出 |
| pixaFindStrokeWidth | ✅ 同等 | `Pixa::find_stroke_width` | 複数線幅検出 |
| pixaModifyStrokeWidth | ✅ 同等 | `Pixa::modify_stroke_width` | 線幅変更 |
| pixModifyStrokeWidth | ✅ 同等 | `modify_stroke_width` | 単一線幅変更 |
| pixaSetStrokeWidth | ✅ 同等 | `Pixa::set_stroke_width` | 線幅設定 |
| pixSetStrokeWidth | ✅ 同等 | `set_stroke_width` | 単一線幅設定 |

**strokes.c カバレッジ**: 7/7 = 100%

### runlength.c (ランレングス変換)
| C関数 | 状態 | Rust対応 | 備考 |
|-------|------|----------|------|
| pixRunlengthTransform | ✅ 同等 | `runlength_transform` | ランレングス変換 |
| runlengthMembershipOnLine | ✅ 同等 | `runlength_membership_on_line` | ライン上のランレングスメンバーシップ |
| makeMSBitLocTab | ✅ 同等 | `make_ms_bit_loc_tab` | MSBビット位置テーブル生成 |

**runlength.c カバレッジ**: 3/3 = 100%

### checkerboard.c (チェッカーボード検出)
| C関数 | 状態 | Rust対応 | 備考 |
|-------|------|----------|------|
| pixFindCheckerboardCorners | ✅ 同等 | `find_checkerboard_corners` | チェッカーボードコーナー検出 |

**checkerboard.c カバレッジ**: 1/1 = 100%

### convertfiles.c (ファイル変換)
| C関数 | 状態 | Rust対応 | 備考 |
|-------|------|----------|------|
| convertFilesTo1bpp | ✅ 同等 | `convert_files_to1bpp` | ファイルを1bppに変換 |

**convertfiles.c カバレッジ**: 1/1 = 100%

### finditalic.c (イタリック体検出)
| C関数 | 状態 | Rust対応 | 備考 |
|-------|------|----------|------|
| pixItalicWords | ✅ 同等 | `italic_words` | イタリック体単語検出 |

**finditalic.c カバレッジ**: 1/1 = 100%
