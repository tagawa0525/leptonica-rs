...
**カバレッジ**: 77.4% (941/1215 関数が実装済み、🚫 不要 146 関数を除くと実質 941/1069 = 88.0% 実装)
...
| numaGetIArray        | 🔄   | `(idiomatic)`                            | iter().map(\                                                            | f\ | *f as i32).collect() で代替 |
...
| numaConvertToInt                | 🔄   | `(idiomatic)`                                         | iter().map(\                  | f\ | f as i32).collect() で代替可 |
