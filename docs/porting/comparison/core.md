...
**カバレッジ**: 87.9% (809/919 関数が実装済み、🚫 不要 77 関数を除くと実質 809/842 = 96.1% 解決済み)
...
| boxaTransformOrdered | ❌ 未実装 | - | |
| boxTransformOrdered | ❌ 未実装 | - | |
| boxaRotateOrth | ❌ 未実装 | - | |
| boxRotateOrth | ❌ 未実装 | - | |
| boxaShiftWithPta | ❌ 未実装 | - | |
| boxaSort | 🔄 | `Boxa::sort_by_position()` / `sort_by_area()` | ソートタイプ別に個別メソッド化 |
| boxaBinSort | ❌ 未実装 | - | |
| boxaSortByIndex | ❌ 未実装 | - | |
| boxaSort2d | ❌ 未実装 | - | |
| boxaSort2dByIndex | ❌ 未実装 | - | |
| boxaExtractAsNuma | ❌ 未実装 | - | |
| boxaExtractAsPta | ❌ 未実装 | - | |
| boxaExtractCorners | ❌ 未実装 | - | |
| boxaGetRankVals | ❌ 未実装 | - | |
| boxaGetMedianVals | ❌ 未実装 | - | |
| boxaGetAverageSize | ❌ 未実装 | - | |
| boxaaGetExtent | ❌ 未実装 | - | Boxa::get_extent()はあるがBoxaaには未実装 |
| boxaaFlattenToBoxa | ✅ 同等 | `Boxaa::flatten()` | |
| boxaaFlattenAligned | ❌ 未実装 | - | |
| boxaEncapsulateAligned | ❌ 未実装 | - | |
| boxaaTranspose | ❌ 未実装 | - | |
| boxaaAlignBox | ❌ 未実装 | - | |
...
| C関数 | 状態 | Rust対応 | 備考 |
|-------|------|----------|------|
| boxaSmoothSequenceMedian | ❌ 未実装 | - | |
| boxaWindowedMedian | ❌ 未実装 | - | |
| boxaModifyWithBoxa | ❌ 未実装 | - | |
| boxaReconcilePairWidth | ❌ 未実装 | - | |
| boxaSizeConsistency | ❌ 未実装 | - | |
| boxaReconcileAllByMedian | ❌ 未実装 | - | |
| boxaReconcileSidesByMedian | ❌ 未実装 | - | |
| boxaReconcileSizeByMedian | ❌ 未実装 | - | |
| boxaPlotSides | ❌ 未実装 | - | |
| boxaPlotSizes | ❌ 未実装 | - | |
| boxaFillSequence | ❌ 未実装 | - | |
| boxaSizeVariation | ❌ 未実装 | - | |
| boxaMedianDimensions | ❌ 未実装 | - | |
