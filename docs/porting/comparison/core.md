...
**カバレッジ**: 87.7% (1065/1215 関数が実装済み、🚫 不要 150 関数を除くと実質 1065/1065 = 100% 実装)
...
| compareTilesByHisto            | ✅   | `compare_tiles_by_histo` (core/pix/compare.rs)              | plan 142                   |
...
| pixCompareGrayByHisto          | ✅   | `pix_compare_gray_by_histo` (core/pix/compare.rs)           | plan 143                   |
| pixComparePhotoRegionsByHisto  | ✅   | `pix_compare_photo_regions_by_histo` (core/pix/compare.rs)  | plan 144                   |
...
| pixDecideIfPhotoImage          | ✅   | `pix_decide_if_photo_image` (core/pix/compare.rs)           | plan 144 (text-check 省略) |
| pixGenPhotoHistos              | ✅   | `pix_gen_photo_histos` (core/pix/compare.rs)                | plan 144                   |
...
| pixaComparePhotoRegionsByHisto | ✅   | `pixa_compare_photo_regions_by_histo` (core/pix/compare.rs) | plan 144                   |
...
| fpixAffine                  | ✅   | `fpix_affine` (core/fpix/extended.rs)                    | plan 140                                                   |
| fpixAffinePta               | 🚫   | -                                                        | `fpix_add_slope_border` 依存; `fpix_affine` を直接使用     |
...
| fpixProjective              | ✅   | `fpix_projective` (core/fpix/extended.rs)                | plan 140                                                   |
| fpixProjectivePta           | 🚫   | -                                                        | `fpix_add_slope_border` 依存; `fpix_projective` を直接使用 |
...
| pixComponentFunction        | ✅   | `pix_component_function` (core/fpix/extended.rs)         | plan 141                                                   |
...
| pixPlotAlongPta        | 🚫   | -                                                  | gplot 依存のデバッグ可視化; Rust 側は plotters 等で代替 |
...
| ptaaGetBoundaryPixels  | ✅   | `ptaa_get_boundary_pixels` (core/pta/graphics.rs)  | plan 139                                                |
