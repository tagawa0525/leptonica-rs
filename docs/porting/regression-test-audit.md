...
生成日: 2026-03-01 (A/B/C/D 分類 + 一覧表の機械カウント)
更新: 2026-05-20 (本書冒頭のサマリ + C 互換性検証への参照を追加)

> **本書のスコープ**: 各 C 版 reg test に対する Rust 側のカバレッジ
> (チェック数、未実装関数) を **意味的解析** で記録した監査表。
> 2026-03 時点のスナップショットで、Rust 側のテスト追加は本書に反映
> されていない箇所がある。一方、**pixel-level の C/Rust 一致状況**
> (Ok / Mismatch / MissingC / Unmapped) は plan 901 の一連の作業 (PR
> #377〜#391) で別の仕組みに移行しており、`c-compat-status.md` で継続的
> に追跡される。
...
| **合計**              | **149**  | `alltests_reg.c` に register された自動実行対象 (本書集計母数) |

## 対象テスト数の見方 (2026-05-20 追記)

`prog/*_reg.c` の総数と本書 A/B/C/D 表の集計対象は 1:1 ではない:

| 観点                                    | 件数 | 説明                                                         |
| --------------------------------------- | ---: | ------------------------------------------------------------ |
| `prog/*_reg.c` の総数                   |  160 | `alltests_reg.c` 含む                                        |
| `alltests_reg` で登録される自動実行対象 |  149 | `scripts/audit-regression-tests.py` の集計母数 = 本書の表    |
| `alltests_reg.c` 自体                   |    1 | テストランナー、本書対象外                                   |
| 差分 (alltests_reg に未登録)            |   10 | binmorph2 / dwamorph2 / fmorphauto / morphseq / binmorph6 等 |

Rust 側 `tests/**/*_reg.rs` は alltests_reg 登録 149 + 未登録 10 + Rust
独自 85 = **244 件** で、本書には未登録 10 件・独自 85 件は載っていない。

> 本書の A/B/C/D 分類は 2026-03 時点の LLM 意味解析。本セッション
> (2026-05-20) では `regression-test-audit.csv` の機械カウント部分のみ
> 再生成し、本書の A/B/C/D 表は当時のままにしている (LLM 再解析は別 PR)。
