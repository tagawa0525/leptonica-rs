...
| **合計**              | **149**  | 2026-03 時点。`alltests_reg` を除く実対象は 159 (差分 10 件は本書に未反映、再生成は TODO) |

> 2026-03 → 2026-05 で C 版に **新規 10 件**が追加され (差分は
> `regression-test-audit.csv` の機械カウント再生成で出る予定)、Rust
> 側もすべて移植済み。本書の A/B/C/D 分類は本セッションでは更新せず、
> `scripts/audit-regression-tests.py` の再実行で更新する別 PR で対応。
