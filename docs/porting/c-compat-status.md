...
>
> **plan 902 PR 2 (dither ペア追加 + kernel 修正) 後**: dither の semantic
> ペア 4 件を追加 (Unmapped 447 → 445、Mismatch 29 → 33)。この過程で
...
| 状態        |    件数 | 説明                                                                                                                                                                          |
| ----------- | ------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ✅ Ok       | **294** | C 版と pixel-level 完全一致 (Phase 2.5 で +10、Phase 3 で +12、plan 902 で +250)                                                                                              |
...
合計 812 entries がレポート対象 (As of 2026-08-18、plan 902 PR 32 後)。
Rust manifest (`tests/golden_manifest.tsv`) 全体は **819 entries**。
