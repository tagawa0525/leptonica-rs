...
> **As of 2026-05-20** (Phase 3 第二弾 merge 後)。Phase 2.5 修正対象
> 10 件すべて解消 (PR #397/#398/#400)。Phase 3 第一弾で hash-match
> ペア 8 件追加 (core/io/transform binary 初登場)、第二弾で
> region/seedspread の semantic ペア 6 件追加 (新 finding 006 で
> Mismatch を可視化、region binary 初登場)。全体 Ok の推移は
> **22 (Phase 2.5 開始時) → 32 (Phase 2.5 完了時) → 40 (Phase 3 第一
> 弾完了時) → 40 (第二弾、Mismatch のみ +6)**。Mismatch は 21 → 27
> (新規 6 件は finding 006 で根本原因調査中、既存 21 件は JPEG codec
> 差で finding 001 参照)。
...
| ⚠️ Mismatch |  **27** | 内訳: 既知の JPEG codec 差 21 件 (finding 001) + seedspread 6 件 (finding 006、第二弾で新規可視化) |
| ⛔ MissingC |   **0** | (PR #381 / Phase 1.5 で解消)                                                                       |
| 📭 Unmapped | **506** | `scripts/golden_map.tsv` 未登録 (Phase 3 進行中、520 → 506、14 件追加マップ)                       |
...
### Phase 3 第二弾で可視化 (6 件、要追加調査)

| カテゴリ     | 件数 | 根拠                                                                          |
| ------------ | ---: | ----------------------------------------------------------------------------- |
| `seedspread` |    6 | [006](c-compat-findings/006-seedspread-output-diff.md): 仮説段階 (要切り分け) |

`seedspread` 6 件は Phase 3 第二弾で `golden_map.tsv` に semantic
マッピングを追加した結果、初めて Mismatch として観測された。Rust と
C で出力 pixel 値が異なる (PNG codec 差ではなく内部 algorithm or
RGBA レイアウト差と推定)。finding 006 で root cause 候補を 3 つ列挙、
別 PR で切り分け。
...
(2026-05-20 第一弾で 520 → 512、第二弾で 512 → 506)。

優先度の高い拡張候補 (現状 Unmapped が多い領域):

- `color` (114), `filter` (97), `transform` (78), `region` (72), `io` (57)
...
- 残 Unmapped 506 件のうち、Rust と C の `*_reg.{rs,c}` の write 順序

  が semantic 一致する prefix を 1 module ずつ追加 (現状の優先順位:
  region 残り → io → transform → filter → color)

- 各追加で Mismatch が増えたら新 finding として記録 (Phase 2.5 と
...
- Phase 2.5 findings (001-005)、Phase 3 finding 006
